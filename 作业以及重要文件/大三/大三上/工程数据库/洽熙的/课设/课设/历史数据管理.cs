...
namespace 课设
{
    public partial class 历史数据管理 : Form
    {
        public 历史数据管理()
        {
            InitializeComponent();
        ...
        private void 历史数据管理_FormClosing(object sender, FormClosingEventArgs e)
        {
            普通用户操作窗体 f = new 普通用户操作窗体();
            f.Show();
            this.Dispose(); 
        }

        private void 历史数据管理_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (textBox2.Text == "")
            {
                MessageBox.Show("请指定时间段", "注意", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                textBox2.Focus();
                return;
            }
            if (textBox3.Text == "")
            ...
            try
            {
                string queryStr = "select * from 测量记录表 where 监测点编号 = '" + jcdbh + "'and  (测量时间 between '" + textBox2.Text + "' and '" + textBox3.Text + "') order by 记录编号 desc;";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string updateStr = "delete from 测量记录表 where 监测点编号 = '" + jcdbh1 + "'and 测量时间 between '" + textBox2.Text + "' and '" + textBox3.Text + "' order by 记录编号 desc;";
